...
# auth_client and create_employee fixtures are defined in conftest.py.
 
 
@pytest.mark.django_db
class TestSalaryMetricsByCountry:
    def test_returns_200_for_existing_country(self, auth_client, create_employee):
        create_employee(country="India", salary=Decimal("80000.00"))
        url = reverse("salary-metrics-by-country") + "?country=India"
        response = auth_client.get(url)
        assert response.status_code == status.HTTP_200_OK
 
    def test_returns_404_for_unknown_country(self, auth_client):
        url = reverse("salary-metrics-by-country") + "?country=Narnia"
        response = auth_client.get(url)
        assert response.status_code == status.HTTP_404_NOT_FOUND
 
    def test_returns_400_when_country_param_missing(self, auth_client):
        url = reverse("salary-metrics-by-country")
        response = auth_client.get(url)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
 
    def test_returns_correct_min_max_avg(self, auth_client, create_employee):
    ...
    def test_returns_correct_country_in_response(self, auth_client, create_employee):
        create_employee(country="India")
        url = reverse("salary-metrics-by-country") + "?country=India"
        response = auth_client.get(url)
        assert response.json()["country"] == "India"
 
    def test_country_matching_is_case_insensitive(self, auth_client, create_employee):
        create_employee(country="India", salary=Decimal("80000.00"))
        url = reverse("salary-metrics-by-country") + "?country=india"
        response = auth_client.get(url)
        assert response.status_code == status.HTTP_200_OK
 
    def test_ignores_employees_from_other_countries(self, auth_client, create_employee):
        create_employee(country="India", salary=Decimal("100000.00"))
    ...
    def test_single_employee_min_max_avg_are_equal(self, auth_client, create_employee):
...
class TestSalaryMetricsByJobTitle:
    def test_returns_200_for_existing_job_title(self, auth_client, create_employee):
        create_employee(job_title="Software Engineer")
        url = reverse("salary-metrics-by-job-title") + "?job_title=Software Engineer"
        response = auth_client.get(url)
        assert response.status_code == status.HTTP_200_OK
 
    def test_returns_404_for_unknown_job_title(self, auth_client):
        url = reverse("salary-metrics-by-job-title") + "?job_title=Astronaut"
        response = auth_client.get(url)
        assert response.status_code == status.HTTP_404_NOT_FOUND
 
    def test_returns_400_when_job_title_param_missing(self, auth_client):
        url = reverse("salary-metrics-by-job-title")
        response = auth_client.get(url)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
 
    def test_returns_correct_average_salary(self, auth_client, create_employee):
    ...
    def test_job_title_matching_is_case_insensitive(self, auth_client, create_employee):
        create_employee(job_title="Software Engineer", salary=Decimal("100000.00"))
        url = reverse("salary-metrics-by-job-title") + "?job_title=software engineer"
        response = auth_client.get(url)
        assert response.status_code == status.HTTP_200_OK
 
    def test_ignores_employees_with_different_job_titles(self, auth_client, create_employee):
        create_employee(job_title="Software Engineer", salary=Decimal("100000.00"))
    ...
    def test_returns_job_title_in_response(self, auth_client, create_employee):
        create_employee(job_title="Software Engineer")
        url = reverse("salary-metrics-by-job-title") + "?job_title=Software Engineer"
        response = auth_client.get(url)
